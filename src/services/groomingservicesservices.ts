...
export class GroomingServicesService {
  ...
  addGroomingService(groomingService: GroomingServices): Observable<GroomingServices> {
    const headers = this.getHeaders();
    return this.http.post<GroomingServices>(`${this.baseUrl}/add`, groomingService, { headers });
  }

  updateGroomingService(serviceId: number, groomingService: GroomingServices): Observable<GroomingServices> {
    const headers = this.getHeaders();
    return this.http.put<GroomingServices>(`${this.baseUrl}/${serviceId}`, groomingService, { headers });
  }
}